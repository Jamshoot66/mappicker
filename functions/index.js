...
/** updateMission
 *  POST
 *  requset = url/updateMission
 *      headers:
 *          - authorization with token
 *      body:
 *          - "mission" - mission json
 *  response = 
 *      - status 200 + { response: 'Mission updated' }
 *      - status 400 + payload on error
 */
exports.updateMission = functions.https.onRequest((req, res) => {
  ...
  cors(req, res, () => {
    ...
    admin.auth().verifyIdToken(req.headers.authorization).then((decodedToken) => {
      ...
      if (user.canAdd) {
        let sanitizedMission = Object.assign({}, dbTypes.default.defMission, req.body.mission);
        //add mission
        return db.collection(dbTypes.collections.missions).doc(sanitizedMission.guid).update(sanitizedMission)
      } else {
        throw new Error(JSON.stringify({ err: "you cant add or update missions" }));
      }
    }).then((ref) => {
      return res.send(JSON.stringify({ response: 'Mission updated' }));
    }).catch(err => {
      return res.status(400).send(JSON.stringify({ err: err.message }));
    });
