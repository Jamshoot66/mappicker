...
class App extends React.Component {
    ...
    let callback = async firebaseUser => {
      if (firebaseUser != null) {
        ...
        try {
          ...
          if (user.rights.canRead) {
            await this.props.getAllMissions();
          }
          if (user.rights.canAdd) {
  ...
  async componentDidUpdate(prevProps) {
    if (
      this.props.user.rights.canRead !== prevProps.user.rights.canRead ||
  ...
  render() {
    return (
      <div>
        {this.props.page === actionType.pages.INIT
          ? loader(
              utils.initFileList,
              () => {
                this.props.setPage(actionType.pages.PRELOAD);
              },
              0
            )
          : null}
        {this.props.page === actionType.pages.PRELOAD ? <LoaderFiller /> : null}
        {this.props.page === actionType.pages.PRELOAD
          ? loader(
              utils.preloadFileList,
              () => {
